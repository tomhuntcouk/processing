...
class PointList {
	...
	noise( amplitude, frequency=1, time=0 ) {
		// console.log(amplitude, frequency, time);
		for( let vert of this.vertices ) {
			let nx = noise( (vert.x+width) * frequency, (vert.y+height) * frequency, time );
			let ny = noise( (vert.y+width) * frequency, (vert.x+height) * frequency, time );
			nx = (nx - 0.5 ) * 2;
			ny = (ny - 0.5 ) * 2;
			vert.add( new TransMatrix.Vector3( nx * amplitude, ny * amplitude ) );
		}
	}
